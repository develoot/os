...
#define FORMAT_STRING_CONVERSION_BUFFER_SIZE (1024)
...
int string_format(char *output_buffer, size_t output_buffer_size, const char *format, ...)
{
    ...
    while (*format_cursor != '\0') {
        ...
        case 'c': {
            ...
            output_buffer[output_buffer_index++] = (char)va_arg(va, int);
            ++format_cursor;
            break;
        }
        case 's': {
            const char *const string = va_arg(va, char *);
        ...
        case 'd': {
            size_t string_size = convert_int_to_string(conversion_buffer, va_arg(va, int32_t));
            if (output_buffer_size < output_buffer_index + string_size) {
                return -1;
            }
            output_buffer_index
                += copy_string_to_buffer(&output_buffer[output_buffer_index], conversion_buffer);
            ++format_cursor;
            break;
        }
        case 'u': {
            size_t string_size = convert_uint_to_string(conversion_buffer, va_arg(va, uint32_t));
            if (output_buffer_size < output_buffer_index + string_size) {
                return -1;
            }
            output_buffer_index
        ...
        case 'l': {
            ...
            switch (*format_cursor) {
            case 'd': {
                size_t string_size = convert_long_to_string(conversion_buffer, va_arg(va, int64_t));
                if (output_buffer_size < output_buffer_index + string_size) {
                    return -1;
                }
                output_buffer_index
            ...
            case 'u': {
                size_t string_size = convert_ulong_to_string(conversion_buffer, va_arg(va, int64_t));
                if (output_buffer_size < output_buffer_index + string_size) {
                    return -1;
                }
                output_buffer_index
